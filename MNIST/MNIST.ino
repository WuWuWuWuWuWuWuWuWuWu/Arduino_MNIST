...
void loop() {
  ...
  Serial.println("index\tpredict\t\tlabel\taccuracy\tlatency");
    ...
    double start_time = millis();
		...
		uint8_t max = 0;
		for(int j= 0; j < 10; j++) if(output->data.uint8[j] > max) max = j;
    time += (millis() - start_time);
    if(max == image_list[i]) correct += 1;
    Serial.println("\t" +String(i + 1) + "\t\t" + String(image_list[i]) + "\t\t" + String(max) + "\t\t" + String(double(correct) / double(i + 1)) + "\t\t" + String(time / double(i + 1)));
